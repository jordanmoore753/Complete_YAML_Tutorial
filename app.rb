...
def create_user_yml(obj) # this method writes a new YML file in the users folder
  path = data_path
...
post "/add_interest" do 
  logged_in?

  @user = find_user(session[:curr_user]) 
  @user.add_interest(params[:interest])
  create_user_yml(@user)
